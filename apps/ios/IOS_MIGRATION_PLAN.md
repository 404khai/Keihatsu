...
This is a planning deliverable, not an implementation phase. It retains the eight phase numbers in [AGENTS.md](Keihatsu/AGENTS.md), but adds live data work to the relevant phases to cover the requested API integration. Phase 0 is an existing foundation; it should not be regenerated. Backend changes described as **proposed** below do not exist yet.
...
| 7 | Supporting features and interaction polish | Bug reports, real statistics, honest feed/notification states | 2–6 |
| 8 | Release candidate with measured performance | End-to-end parity, migration and failure recovery verification | 1–7 |
...
Purpose: deliver the first real-data browsing milestone, retaining Flutter’s scanning rhythm.
...
Each phase record should state: what is functional with real data, what is local-only, what remains a fixture or enhancement, the checks run, and any unresolved API dependency. The broader road map is complete when all eight outcomes are delivered; a UI-only screen does not count as live data parity.

Phase 1 implementation is recorded below. The first live content milestone is Phase 2; the first complete reading milestone is Phase 4; account/offline parity arrives through Phases 5–6.
