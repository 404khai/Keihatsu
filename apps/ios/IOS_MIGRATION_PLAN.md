...
| Details | [MangaDetailView.swift](Keihatsu/Features/MangaDetails/MangaDetailView.swift) has artwork, chapters, category selection, and in-memory read/bookmark state. | Evolve into a feature driven by real manga/chapter identities. |
...
| Profile/community | Editable username/bio and Blobatar configuration, public profile and visibility, API-backed profile totals; reader comments include replies, likes, images, and author profiles. The native ProfileView is intentionally banner-free. | Phase 5, with SwiftUI forms, PhotosPicker, sheets, and shared avatar rendering. |
...
5. **Profile:** fetch real totals, edit username/bio, port deterministic Blobatar hue/shape/expression/animation, public profile and privacy visibility. Keep ProfileView and the native editor banner-free even though the backend retains its Flutter-compatible banner field. Share the avatar component across account and comments. A SwiftUI `Shape`/`Canvas` renderer or a suitable native Blobatar implementation needs fixture comparison with Flutter’s output.
...
Purpose: let people glance at an ongoing download, their manga reading session, or active incognito mode and return directly to the relevant context. This is a required native enhancement, implemented after the real reader, account isolation, and download lifecycle exist. It does not renumber Phases 7–8.
...
| Incognito mode | Start a single incognito Live Activity when the user enables incognito mode. The toggle must not change ProfileView, reader chrome, app theme, banners, tint, or any other in-app presentation; incognito continues to suppress automatic history, progress, reading-time, and related outbox writes. | End and remove the activity immediately when incognito is disabled or the owning account logs out/switches. Tapping opens Privacy settings. |
...
- Implement Lock Screen, compact leading/trailing, minimal, and expanded layouts for all three activity types. Compact download content shows a download symbol and progress/count; compact reading content shows a book symbol and page count. Compact incognito content places the Keihatsu logo in the leading region and the bundled `theater_comedy_outlined` icon at the far-right trailing edge. Minimal content combines recognizable meaning with progress where space permits. Expanded and Lock Screen layouts retain the same hierarchy: title/context, progress, status, then an essential action.
...
1. Add a WidgetKit extension with three `ActivityConfiguration` definitions and shared `DownloadActivityAttributes`, `ReadingActivityAttributes`, and `IncognitoActivityAttributes`. Enable `NSSupportsLiveActivities` in both app build configurations. Share only small value types and appropriately sized local cover assets through an App Group; the extension must not fetch covers from the network. Keep combined static/dynamic activity data below 4 KB and exclude credentials, raw image bytes, and private file URLs. These constraints come from [Apple's ActivityKit implementation guide](https://developer.apple.com/documentation/activitykit/displaying-live-data-with-live-activities).
2. Inject a `LiveActivityCoordinator` through the existing composition root. Reader/download engines and the incognito preference publish snapshots; the coordinator maps them to ActivityKit and handles request/update/end errors without disrupting reading or downloads. Observe authorization and activity-state changes, reconcile `Activity.activities` on relaunch, and prevent duplicate or orphaned activities. Permit at most one aggregate download activity, one reading activity, and one incognito activity; use relevance scores when several exist, while accepting the system's display choice.
...
4. When incognito is enabled, immediately end/remove an existing reading activity and start the incognito activity without making any visual change inside the app. Do not expose manga identity or reading position in the incognito activity. Provide a “Show manga details” privacy preference, defaulting to a generic title and no cover on reading/download system surfaces until enabled. Redact sensitive views where appropriate. End/remove account-owned activities and clear their shared snapshots on logout/account switch. Recheck privacy before every update and deep-link restoration.
...
- Authorization denied/revoked, user dismissal, disabled preferences, activity limits, stale data, account switching, and incognito are covered. Verify the incognito toggle leaves every in-app surface visually unchanged while its Live Activity starts/ends, including the Keihatsu-logo leading and `theater_comedy_outlined` far-right compact presentation. Dismissal does not stop the underlying task; no cross-account title, artwork, or route leakage occurs.
...
1. **Phase Goal** — Complete the manga detail journey on `feat/ios-phase-3-manga-details`, building on the existing hero composition while replacing its static detail and integer-chapter assumptions.
2. **Flutter Files Inspected** — `screens/MangaDetailsScreen.dart`, `services/manga_repository.dart`, `models/manga.dart`, `models/chapter.dart`, and `models/local_models.dart`; the API source routes and current SwiftUI Home/Search/Library entry points were also checked against those behaviors.
3. **Flutter Architecture And Layout Findings** — Flutter loads metadata, chapters, and recommendations as independent futures. Chapters are newest-first for browsing, the first read starts at the oldest chapter, unfinished history resumes its saved chapter/page, and completed history advances chronologically. Its local repository merges refreshed transport fields into durable records without discarding read, bookmark, or download flags.
4. **SwiftUI Adaptation Strategy** — Retain the existing artwork-led dark detail screen, zoom transition, overview, chapter card, swipe actions, and category sheet. Add native pull-to-refresh, independent inline errors/retry, a filter sheet, `ShareLink`, progressive three-chapter disclosure, and a clear Read/Resume control using the existing iOS 26 glass treatment.
5. **Planned Files** — Evolve `MangaDetailView`; add a detail view model and navigation seed, domain reading state/filter/resume types, chapter ordering and resume use cases, a repository contract/implementation, disk store, API endpoint builders, focused tests, and a typed reader-entry boundary. Update Home/Search/Library destinations and app service composition.
6. **Architecture Decisions** — `MangaDetailsViewModel` coordinates three independent reads through `MangaDetailsRepository`. `DefaultMangaDetailsRepository` composes the Phase 2 catalogue contract with `MangaDetailsStore`; the store namespaces records by API origin and composite manga identity. UI routes carry `MangaDetailsSeed`, while `ReaderLaunchContext` carries composite chapter identity, origin, and zero-based saved page. Static cards are converted once at the feature boundary for preview continuity.
7. **SwiftUI Mapping Notes** — The prior 620-point hero and content rhythm remain. Material filter controls map to a native sheet with composable toggles; share maps to `ShareLink`; the chapter section keeps the previous native `List` behavior so bookmark/read swipe actions remain available, with larger touch targets and inset custom dividers. Recommendations use the same cover pipeline and push another domain detail seed. Library/category actions show an account gate in live composition; preview composition can exercise the category UI.
8. **Implementation** — Live metadata and chapters now load concurrently and render as each succeeds; same-source popular recommendations load separately. Cached detail records hydrate first. Fractional chapters sort numerically and specials sort deterministically. Filters compose downloaded, unread, and bookmarked flags. Read/Resume follows oldest, unfinished, and finished-history rules. Refreshes preserve local state on disk. Home, Search, Library, and recommendation routes preserve their launch origin and reader position. Valid source URLs expose native sharing.
9. **Phase Summary** — Live catalogue items now open a complete API-backed detail experience and reach a typed reader entry. The existing visual design was extended rather than rewritten. The durable detail store is ready for the reader and download phases to update page and file state.
10. **Files Created/Modified** — `App/AppServices.swift`; `Data/API/MangaAPI.swift`; `Data/Persistence/MangaDetailsStore.swift`; `Data/Repositories/{DefaultMangaDetailsRepository,LiveCatalogueRepository}.swift`; `Domain/Contracts/MangaDetailsRepository.swift`; `Domain/Entities/MangaDetails.swift`; `Domain/UseCases/{ChapterOrdering,ResolveResumeChapter}.swift`; `Features/MangaDetails/MangaDetailView.swift`, its `Models/MangaDetailsSeed.swift` and `ViewModels/MangaDetailsViewModel.swift`; `Features/Reader/Views/{ReaderEntryView,ReaderView}.swift`; Home/Search/Library views; unit/UI tests; and this plan. Unrelated `monorepo.md` and `output/` workspace changes remain outside the phase commit.
11. **Deferred Items** — `ReaderEntryView` intentionally stops at the typed handoff; remote page fetching, image-window management, continuous chapter append, durable reading progress flush, local History updates, and bundled Ordeal page integration remain Phase 4. Account-backed library/category writes remain Phase 5 and physical downloads remain Phase 6. The detail store already retains these flags without claiming those later systems are complete.
12. **Commit Message** — `feat(ios): load manga details and preserve reader context`
13. **Awaiting Approval** — Implementation is limited to the requested Phase 3. Phase 4 has not started.

**Phase 3 validation (6 September 2026)**

- A clean iPhone 17 Pro / iOS 26.5 shared-scheme run passed **24 unit tests in 4 suites and 5 UI tests**, including five Phase 3 unit tests for fractional/special ordering, composed filters, resume selection, reader context, and disk restoration with refresh-state preservation.
- The new UI flows pass from Library card → manga details → oldest-chapter reader entry and verify that chapter rows expose working Mark Read and Bookmark swipe actions. Existing onboarding, navigation placement, toolbar/sheet routes, Library layouts, and category controls also pass.
- Against the running `apps/api` backend, a latest ManhuaTop title returned HTTP 200 for metadata and HTTP 200 for its chapter list in independent requests; the observed list contained 88 chapters. These provider-dependent counts are recorded evidence rather than test expectations.
- `git diff --check` passed. Final result bundle: `/tmp/keihatsu-phase3-chapters-full.xcresult`; clean build output: `/tmp/keihatsu-phase3-build-final`.

**Phase 4 implementation record**

1. **Phase Goal** — Replace the placeholder reader handoff with a live, vertically continuous SwiftUI reader and durable local continuity on `feat/ios-phase-4-reader-engine`, while preserving the completed Phase 3 detail design.
2. **Flutter Files Inspected** — `screens/MangaReaderScreen.dart`, its detail resume call sites, `services/{manga_repository,history_repository}.dart`, `components/Comments.dart`, and `providers/download_provider.dart`; the existing API pages route, shared native image pipeline, settings, Home, History, and bundled Ordeal resources were checked against those behaviors.
3. **Flutter Findings** — Flutter loads local files or archives before remote pages, appends the next chronological chapter within 800 points of the end, toggles chrome on tap, exposes a one-based slider plus chapter controls, debounces history writes for three seconds, marks the last page read, and records elapsed time. Its current page estimate is based on a fraction of total scroll extent, so it drifts for uneven page heights; native tracking instead uses visible page geometry and stable IDs.
4. **Architecture Decisions** — `ReaderViewModel` owns presentation and loading coordination; `DefaultReaderRepository` resolves verified local files/bundled fixtures before the existing typed API; `ReaderProgressStore` and `LocalHistoryRepository` own durable progress and detail-state updates; `ReaderSession` owns active-time and typed lifecycle events; `ChapterSequence` owns chronology; and `PagePrefetcher` extends the shared bounded `ImagePipeline`. Automatic recording is blocked before the repository when incognito is active, while explicit bookmarks remain available.
5. **SwiftUI Mapping Notes** — The reader remains an immersive vertical strip with tap-to-toggle controls, native glass top/bottom chrome, a stable-page slider, previous/next actions, bookmark/comments affordances, inline retry, and explicit chapter boundaries. Visible-page selection uses the largest viewport intersection and persists a zero-based page plus intra-page anchor. Only the implemented vertical mode is offered in active settings; background color and keep-awake preferences are honored.
6. **Implementation** — Live `GET /sources/{sourceID}/chapters/{chapterID}/pages` responses render through the same proxy-aware image pipeline as catalogue artwork, with a 48 MiB decoded-image ceiling, shared in-flight work, cancellation, and a two-page prefetch radius. Continuous append deduplicates chapters and retains at most five loaded chapters, restoring the current stable anchor when pruning. Progress flushes after two seconds, on background, and on exit; active time excludes suspended periods, read state is sticky after the final page, and opening/prefetching alone does not create history. Durable entries feed new Continue Reading and Recent Reading surfaces. Bundled Ordeal pages remain an offline stress fixture. Chapter rows were reduced from 96 to 88 points with 16-point vertical padding while retaining the larger download target and both swipe actions.
7. **Files Created/Modified** — Reader domain entities/contracts; `ReaderProgressStore`; `DefaultReaderRepository`; `LocalHistoryRepository`; reader view model, session, sequence, prefetcher, page/chrome/comments components, and entry view; shared image pipeline and app composition; Home, History, reader settings, manga detail entry, chapter row sizing; unit/UI tests; and this plan.
8. **Deferred Items** — The comments sheet keeps its entry point but authenticated threads remain Phase 5. CBZ extraction and durable offline archives remain Phase 6; the repository already prioritizes verified local page directories. ActivityKit will consume the typed reader session events in Phase 6A. Horizontal/paged modes and brightness controls remain outside production settings until their engines exist.
9. **Commit Message** — `feat(ios): add live reader and durable reading progress`
10. **Awaiting Approval** — Implementation is limited to the requested Phase 4. Phase 5 has not started.

**Phase 4 validation (6 September 2026)**

- A clean iPhone 17 Pro / iOS 26.5 shared-scheme run passed **29 unit tests in 6 suites and 6 UI tests**, covering chapter chronology, exact page/anchor restoration, detail-state propagation, final-page read marking, incognito suppression, explicit bookmarks, bundled-page rendering, reader controls, chapter swipe actions, onboarding, and preserved navigation/collection flows.
- The reader UI performance case records memory and scroll-deceleration baselines while traversing the bundled variable-height Ordeal fixture: five scroll samples averaged about 2.30 seconds and peak physical memory ranged from 68.2–68.4 MB on the simulator. The result bundle and screenshot are retained at `/tmp/keihatsu-phase4-full.xcresult`.
- Against the running `apps/api` backend, ManhuaTop returned 18 latest titles, 50 chapters for the observed title, and 11 pages for its oldest chapter. The first page returned a valid 800×533 WebP through `/sources/proxy/image` (143,450 bytes). Provider-dependent counts are evidence rather than test expectations.
- `git diff --check` passed. Clean build output: `/tmp/keihatsu-phase4-build`.

**Phase 5 implementation record**

1. **Phase Goal** — Connect Google authentication, durable account-scoped collection/history sync, profiles, privacy, supported preferences, and reader comments without replacing the library-first native layout.
2. **Flutter Files Inspected** — `auth_provider.dart`, `auth_api.dart`, `LoginScreen.dart`, `RegisterScreen.dart`, `session_bootstrap_service.dart`, account models/repositories, `sync_manager.dart`, profile/privacy/Blobatar screens, and comments provider/components were compared with their API call sites.
3. **Flutter Architecture And Layout Findings** — Sign-in and sign-up share Google token exchange; authenticated bootstrap reads account/stats/preferences; offline mutations need owner scope and dependency ordering; comments are optional-auth reads with multipart authenticated writes. The native ProfileView intentionally omits Flutter's banner.
4. **SwiftUI Adaptation Strategy** — Keep the established tab/profile composition and use native Google Sign-In, Keychain, `PhotosPicker`, sheets, swipe actions, and a shared Canvas avatar. Incognito still suppresses automatic reading records but produces no in-app appearance change; its system presentation is specified for Phase 6A.
5. **Planned Files** — Authentication/account domain and API adapters, account cache and durable outbox, sync coordinator, profile/public-profile/editor views, shared avatar, comments view model/sheet, app composition, backend auth/library/category/history/comment contracts, Prisma schema/migration, tests, and this plan.
6. **Architecture Decisions** — Credentials live only in Keychain. Cached account and collection data are namespaced by API origin and owner. Outbox records carry typed operations and owner IDs; dependent server IDs resolve before category assignment. Bootstrap is generation-checked and coalesced, 401 and transport failures remain distinct, and guest state is isolated from every account snapshot.
7. **SwiftUI Mapping Notes** — Account entry uses one Google action plus guest continuation. Profile is banner-free and shows real account totals, deterministic Blobatar, editor, privacy, sign-in/out, and an incognito toggle with no theme/banner transformation. Comments retain reader position in a sheet and support nested replies, likes, deletion, author profiles, and five selected images.
8. **Implementation** — Added source-aware library/history/comment identities, full-set category assignment, timestamp conflict handling, history snapshots/tombstones, transactionally deduplicated reading-time operation IDs, direct preference responses, and an accepted iOS Google audience. Native bootstrap restores cache first, reconciles categories/library/paginated history/preferences, preserves pending local intent, and syncs real acknowledgements. Profile, privacy, public profiles, and comments now consume live APIs.
9. **Phase Summary** — Phase 5 account and community flows are implemented across the native client and supporting API contracts. The app builds with Google Sign-In 9.2.0 and retains useful cached account state when transport is unavailable.
10. **Files Created/Modified** — `Core/Networking/KeychainTokenStore.swift`; `Data/API/{AuthAPI,LibraryAPI,HistoryAPI,CategoriesAPI,UserAPI,CommentsAPI}.swift`; account DTOs/entities/contracts/cache/store/coordinator; authentication, profile, avatar, comments, Library/History/settings/app composition; Xcode package/configuration; API auth/library/category/history/comments/preferences services and DTOs; Prisma schema/migration; unit/contract tests; and this plan.
11. **Deferred Items** — Physical Google sign-in needs project OAuth values and a registered reversed-client callback scheme. Live API migration/deployment, returning Flutter-account smoke tests, physical-device accessibility checks, downloads, and all three Live Activities remain deployment or later-phase work. Phase 6A now explicitly owns incognito Live Activity presentation with the Keihatsu logo leading and `theater_comedy_outlined` at the far-right compact edge.
12. **Commit Message** — `feat(ios): sync accounts collections and reading activity`
13. **Awaiting Approval** — Implementation is limited to requested Phase 5 plus the requested Phase 6A plan clarification. Phase 6 and ActivityKit implementation have not started.

**Phase 5 validation (6 September 2026)**

- A clean iPhone 17 Pro / iOS 26.5 simulator build succeeded. The iOS unit suite passed **35 tests**, including account/guest isolation, durable owner-scoped outboxes, explicit preference decoding, history tombstones, nested comments, and existing reader behavior.
